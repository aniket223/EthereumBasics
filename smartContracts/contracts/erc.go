...
// TokenTransfclearerIterator is returned from FilterTransfclearer and is used to iterate over the raw logs and unpacked data for Transfclearer events raised by the Token contract.
type TokenTransfclearerIterator struct {
	Event *TokenTransfclearer // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *TokenTransfclearerIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(TokenTransfclearer)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *TokenTransfclearerIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *TokenTransfclearerIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// TokenTransfclearer represents a Transfclearer event raised by the Token contract.
type TokenTransfclearer struct {
	From   common.Address
	To     common.Address
	Tokens *big.Int
	Raw    types.Log // Blockchain specific contextual infos
...
// FilterTransfclearer is a free log retrieval operation binding the contract event 0x5ff20ed8d1485136511cc076b6e30a3ea4efa3ef2ab5287fcd6b673aedb86bce.
//
// Solidity: event Transfclearer(address indexed from, address indexed to, uint256 tokens)
func (_Token *TokenFilterer) FilterTransfclearer(opts *bind.FilterOpts, from []common.Address, to []common.Address) (*TokenTransfclearerIterator, error) {
	...
	logs, sub, err := _Token.contract.FilterLogs(opts, "Transfclearer", fromRule, toRule)
	if err != nil {
		return nil, err
	}
	return &TokenTransfclearerIterator{contract: _Token.contract, event: "Transfclearer", logs: logs, sub: sub}, nil
}

// WatchTransfclearer is a free log subscription operation binding the contract event 0x5ff20ed8d1485136511cc076b6e30a3ea4efa3ef2ab5287fcd6b673aedb86bce.
//
// Solidity: event Transfclearer(address indexed from, address indexed to, uint256 tokens)
func (_Token *TokenFilterer) WatchTransfclearer(opts *bind.WatchOpts, sink chan<- *TokenTransfclearer, from []common.Address, to []common.Address) (event.Subscription, error) {
	...
	logs, sub, err := _Token.contract.WatchLogs(opts, "Transfclearer", fromRule, toRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(TokenTransfclearer)
				if err := _Token.contract.UnpackLog(event, "Transfclearer", log); err != nil {
					return err
				}
				event.Raw = log
